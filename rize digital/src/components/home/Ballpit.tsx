...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL failed to initialize:", err);
      setHasError(true);
    }
  ...
  if (hasError) {
    return (
      <div className={`${className} bg-[#060218] overflow-hidden`} style={{ width: '100%', height: '100%', position: 'absolute', inset: 0 }}>
        {/* CSS Bubble Fallback */}
        <style>{`
          @keyframes float-bubble {
            ...
            10% { opacity: 0.55; }
            90% { opacity: 0.55; }
            100% { transform: translateY(-20%) scale(1.6); opacity: 0; }
          }
        `}</style>
        {[...Array(30)].map((_, i) => {
          const size = Math.random() * 65 + 20;
          const left = Math.random() * 100;
          const duration = Math.random() * 12 + 8;
          const delay = Math.random() * -10; // Negative delay to start immediately
          const color = ['#ff5722', '#1a56db', '#22c55e'][i % 3];
          return (
            <div
              key={i}
