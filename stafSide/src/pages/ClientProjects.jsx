  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-xl bg-white rounded-3xl border border-gray-200 p-8 shadow-2xl"
      >
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-black flex items-center gap-2">
            <User className="text-blue-500" /> Edit Client Details
          </h3>
          <button onClick={onClose} className="p-2 hover:bg-black/5 rounded-xl text-gray-800">
            <X size={20} />
          </button>
        </div>
...
const parseGraphicWorkDetail = (workDetail = '') => {
  const result = { reels: '', posts: '', shoots: '', perReelAmount: '', perPostAmount: '', perShootAmount: '' };
  if (!workDetail) return result;
  ...
  for (const line of lines) {
    ...
    const rateReelMatch = line.match(/Rate\s+Per\s+Reel:\s*₹?(\d+(\.\d+)?)/i);
    if (rateReelMatch) {
      result.perReelAmount = rateReelMatch[1];
    }

    const ratePostMatch = line.match(/Rate\s+Per\s+Post:\s*₹?(\d+(\.\d+)?)/i);
    if (ratePostMatch) {
      result.perPostAmount = ratePostMatch[1];
    }

    const rateShootMatch = line.match(/Rate\s+Per\s+Shoot:\s*₹?(\d+(\.\d+)?)/i);
    if (rateShootMatch) {
      result.perShootAmount = rateShootMatch[1];
    }
  }
  return result;
};
...
const makeDeptData = (dept) => {
  if (dept === 'SMM') {
    ...
    const fields = SMM_PACKAGE_FIELDS[pkg] || { reels: '10', posts: '6', shoots: '3', accountsCount: '2', accountsList: 'Facebook, Instagram' };
    ...
    return { package: pkg, smmFields: fields, workDetail, amount: (fee * 1.18).toFixed(0) };
  }
  if (dept === 'SEO' || dept === 'PPC') {
    const pkg = 'Starter Package';
    const fee = PACKAGE_DETAILS[pkg]?.fee || 10000;
    return { package: pkg, workDetailsList: [''], workDetail: '', amount: (fee * 1.18).toFixed(0) };
  }
  if (dept === 'Graphic Design & Video Editing') {
    return { package: 'Custom Graphic Package', graphicFields: { reels: '', posts: '', shoots: '', perReelAmount: '', perPostAmount: '', perShootAmount: '' }, workDetail: '', amount: '0' };
  }
...
const EditProjectModal = ({ isOpen, onClose, project, onSave }) => {
  ...
  useEffect(() => {
    if (project && isOpen) {
      const rawDeptStr = project.department || 'WEB DEvlopment';
      const depts = rawDeptStr.split(',').map(d => d.trim()).filter(d => ALL_DEPTS.includes(d));
      const activeDepts = depts.length > 0 ? depts : ['SMM'];
      setSelectedDepts(activeDepts);

      const rawWorkDetail = project.workDetail || '';

      const deptSections = {};
      const sectionMatches = [...rawWorkDetail.matchAll(/---\s*(.*?)\s*---\n([\s\S]*?)(?=(?:---\s*.*?\s*---|$))/g)];
      if (sectionMatches.length > 0) {
        sectionMatches.forEach(m => {
          const headerName = m[1].trim();
          const content = m[2].trim();
          const matchedDept = ALL_DEPTS.find(d => d.toLowerCase() === headerName.toLowerCase() || DEPT_CONFIG[d]?.label.toLowerCase() === headerName.toLowerCase());
          if (matchedDept) {
            deptSections[matchedDept] = content;
          }
        });
      } else {
        deptSections[activeDepts[0]] = rawWorkDetail;
      }

      const initialDeptWorkData = {};
      ALL_DEPTS.forEach(dept => {
        const deptText = deptSections[dept] || (activeDepts.includes(dept) ? rawWorkDetail : '');
        if (dept === 'SMM') {
          const smmFields = parseSmmWorkDetail(deptText);
          const pkg = project.package || 'Sliver Package Service';
          const fee = PACKAGE_DETAILS[pkg]?.fee || 15000;
          const r = smmFields.reels || '0'; const p = smmFields.posts || '0';
          const s = smmFields.shoots || '0'; const c = smmFields.accountsCount || '0';
          const l = smmFields.accountsList || '';
          const detail = [
            `Total Posting ${parseInt(r) + parseInt(p)} ( ${r} Reel & ${p} Post )`,
            `${s} Professional shoot`,
            `Accounts Handled: ${c} (${l})`
          ...
          const graphicFields = parseGraphicWorkDetail(deptText);
          const rc = parseInt(graphicFields.reels || '0'); const pc = parseInt(graphicFields.posts || '0');
          const sc = parseInt(graphicFields.shoots || '0');
          const rr = parseFloat(graphicFields.perReelAmount || '0'); const pr = parseFloat(graphicFields.perPostAmount || '0');
          const sr = parseFloat(graphicFields.perShootAmount || '0');
          const subtotal = rc * rr + pc * pr + sc * sr;
          initialDeptWorkData[dept] = {
            package: 'Custom Graphic Package',
            graphicFields,
            workDetail: deptText,
            amount: subtotal > 0 ? (subtotal * 1.18).toFixed(0) : '0'
          };
        } else if (dept === 'WEB DEvlopment') {
          const webPages = parseWebPagesWorkDetail(deptText);
          const pkg = project.package || 'Informative Website';
          const fee = PACKAGE_DETAILS[pkg]?.fee || 15000;
          initialDeptWorkData[dept] = {
            package: pkg,
            webPages,
      ...
      setDeptWorkData(initialDeptWorkData);
      setTotalAmount(project.totalPrice?.toString() || project.totalAmount?.toString() || '');
      setFormData({
  ...
  const computeTotal = (data, selected) => {
    return selected.reduce((sum, d) => sum + (parseFloat(data[d]?.amount) || 0), 0);
  };
    ...
    setSelectedDepts(prev => {
      ...
      if (prev.includes(dept)) {
        ...
        setDeptWorkData(wd => {
          const updated = { ...wd };
          delete updated[dept];
          setTotalAmount(computeTotal(updated, next).toFixed(0));
          return updated;
        });
      } else {
        next = [...prev, dept];
        const newDeptData = makeDeptData(dept);
        setDeptWorkData(wd => {
          const updated = { ...wd, [dept]: newDeptData };
          setTotalAmount(computeTotal(updated, next).toFixed(0));
          return updated;
        });
      }
  ...
  const updateDeptData = (dept, patch) => {
    setDeptWorkData(prev => {
      const updated = { ...prev, [dept]: { ...prev[dept], ...patch } };
      setTotalAmount(computeTotal(updated, selectedDepts).toFixed(0));
      return updated;
    });
  };
      ...
      >
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="text-[10px] font-bold text-gray-700 dark:text-gray-400 uppercase tracking-widest block mb-1.5">Project Status</label>
              <select
                className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-black dark:text-white focus:border-purple-500 outline-none transition-all cursor-pointer"
          ...
          {/* Per-Department Work Detail Cards */}
                  ...
                  {dept === 'SMM' && (
                    <>
                      <div>
                        ...
                        <select className="w-full bg-white dark:bg-[#030303] border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2 text-sm text-black dark:text-white outline-none cursor-pointer" value={data.package || 'Sliver Package Service'} onChange={e => handleSmmPackage(dept, e.target.value)}>
          ...
          {/* Grand Total Bar */}
          {selectedDepts.length > 0 && (
            <div className="flex items-center justify-between bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-500/10 dark:to-indigo-500/10 border border-purple-200 dark:border-purple-500/30 rounded-2xl px-5 py-4">
              <div>
              ...
              <div className="relative flex items-center">
                <span className="absolute left-3 text-xl font-black text-purple-600 dark:text-purple-400">₹</span>
                <input
                  type="number"
                  className="w-40 bg-white dark:bg-[#030303] border border-purple-200 dark:border-white/10 rounded-xl pl-8 pr-3 py-2 text-right text-xl font-black text-purple-600 dark:text-purple-400 focus:border-purple-500 outline-none transition-all"
                  value={totalAmount}
          ...
          <div className="flex gap-3 mt-8">
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-xl font-bold transition-all shadow-lg ${selectedDepts.length === 0 ? 'bg-gray-200 dark:bg-white/10 text-gray-400 cursor-not-allowed shadow-none' : 'bg-purple-600 text-white hover:bg-purple-700 shadow-purple-600/20'}`}
            >
              Save Project
            </button>
          </div>
    ...
    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 ${isHistory ? 'opacity-95' : ''}`}>
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden">
                  ...
                  <AnimatePresence>
                    {invoiceMenuOpen && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-lg z-10 overflow-hidden"
                      >
                        <button
                          onClick={async () => {
                          ...
                          className="block w-full text-left px-4 py-2 text-xs font-bold text-black hover:bg-gray-100"
                        >
                          With GST (18%)
                        </button>
                        <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-8">
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Total Price</p>
              <p className="text-lg font-black text-black">₹{project.totalPrice?.toLocaleString('en-IN')}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Paid</p>
              <p className="text-lg font-black text-emerald-500">₹{project.paidAmount?.toLocaleString('en-IN')}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Pending</p>
              <p className="text-lg font-black text-rose-500">₹{project.pendingAmount?.toLocaleString('en-IN')}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Start Date</p>
              <p className="text-xs font-bold text-gray-900">{project.startDate ? new Date(project.startDate).toLocaleDateString() : '-'}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Deadline</p>
              <p className="text-xs font-bold text-gray-900">{project.deadline ? new Date(project.deadline).toLocaleDateString() : '-'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-8 py-4 border-bottom border-gray-50 flex items-center gap-2 bg-gray-50/50">
            <ListChecks size={16} className="text-blue-500" />
            <span className="text-[10px] font-black text-gray-800 uppercase tracking-[0.2em]">Scope of Work</span>
          </div>
          <div className="p-4">
              ...
              <tbody className="divide-y divide-gray-50">
                {project.workDetail?.split(/[\n•]+/).map(item => item.trim()).filter(i => i).map((item, index) => (
                  <tr key={index} className="group hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 text-xs text-gray-900 flex items-start gap-4">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm h-full">
          ...
          <div className="space-y-4">
            {project.tasks?.map((task, index) => {
              ...
              return (
                <div key={index} className="p-5 rounded-2xl bg-gray-50/50 border border-gray-100 hover:border-blue-500/30 transition-all group/task">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className={`text-sm font-bold block ${progress === 100 ? 'text-gray-600 line-through' : 'text-black '}`}>
                          {task.name}
                        </span>
                        <span className="text-[10px] text-gray-800 font-bold uppercase tracking-widest">
                  ...
                  <div key={`extra-${idx}`} className="p-5 rounded-2xl bg-emerald-500/[0.03] border border-emerald-500/10 hover:border-emerald-500/30 transition-all group/task">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <span className="text-sm font-bold block text-emerald-900">{task.name}</span>
...
const ClientProjects = ({ onBack }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12 pb-20 bg-white p-6 sm:p-8 rounded-3xl min-h-screen text-black shadow-inner">
      {/* Top Header Section */}
      <div className="flex flex-col gap-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <button onClick={handleBack} className="p-2 hover:bg-black/5 rounded-xl text-gray-800 transition-colors">
              <ArrowLeft size={24} />
            </button>
          ...
          {projects.length > 0 && (
            <div className="flex flex-wrap sm:flex-row items-stretch sm:items-center gap-3 w-full justify-end">
              <button
                ...
                className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-white border border-gray-200 text-black font-bold text-sm transition-all hover:bg-gray-50 w-full md:w-auto"
              >
                <User size={18} />
              ...
              <div className="flex flex-wrap sm:flex-row items-stretch sm:items-center gap-2 w-full">
                <input
                  ...
                  className="bg-black/5 border border-gray-200 rounded-xl px-4 py-3 text-sm text-black focus:border-blue-500 outline-none transition-all w-full sm:w-auto"
                ...
                <AnimatePresence>
                  {allProjectsMenuOpen && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-lg z-10 overflow-hidden min-w-[200px]"
                    >
                      <button
                        onClick={async () => {
                        ...
                        className="block w-full text-left px-4 py-2.5 text-xs font-bold text-black hover:bg-gray-100"
                      >
                        With GST (18%)
                      </button>
                      <button
        ...
        <h1 className="text-5xl font-black text-blue-600 lowercase tracking-tight">
          {projects[0]?.name}
        </h1>
      </div>
      ...
      {loading ? (
        <div className="py-20 text-center flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-black text-gray-800 uppercase tracking-widest">Loading Project Details...</p>
        </div>
      ) : (
        <div className="space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">Overall Progress</p>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-black text-blue-500">{overallStats.avgProgress}%</span>
                <span className="text-[10px] font-bold text-gray-600 mb-1 uppercase">Across {overallStats.cycleCount} Months</span>
              </div>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">Total Revenue</p>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-black text-black">₹{overallStats.totalRevenue.toLocaleString('en-IN')}</span>
              </div>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">Total Received</p>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-black text-emerald-500">₹{overallStats.totalPaid.toLocaleString('en-IN')}</span>
              </div>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">Total Pending</p>
              <div className="flex items-end gap-2">
        ...
        {isUpdateProgressOpen && tempProjectData && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="relative w-full max-w-4xl bg-white rounded-3xl border border-gray-200 p-8 shadow-2xl max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-black flex items-center gap-2">
                  <ListChecks className="text-blue-500" /> Update Progress
                </h3>
                <button onClick={() => setIsUpdateProgressOpen(false)} className="p-2 hover:bg-black/5 rounded-xl text-gray-800">
                  <X size={20} />
                </button>
              </div>
              ...
              {tempProjectData.workDetail && (
                <div className="mb-6 p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-200">
                  <h4 className="text-xs font-bold text-indigo-700 uppercase tracking-widest flex items-center gap-2 mb-3">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {tempProjectData.tasks?.map((task, index) => (
                    <div key={index} className="p-4 rounded-2xl bg-gray-50 border border-gray-200">
                      <p className="text-sm font-bold text-black mb-2">{task.name}</p>
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => handleTempTaskUpdate(index, -1)}
                          disabled={task.completed <= 0}
                          className="w-10 h-10 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center font-bold hover:bg-blue-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <Minus size={16} />
                        </button>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {tempProjectData.extraTasks?.map((task, index) => (
                      <div key={index} className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200">
                        <p className="text-sm font-bold text-emerald-900 mb-2">{task.name}</p>
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => handleTempTaskUpdate(index, -1, true)}
                            disabled={task.completed <= 0}
                            className="w-10 h-10 rounded-xl bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold hover:bg-emerald-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            <Minus size={16} />
                          </button>
                          <span className="text-lg font-black text-black flex-1 text-center">{task.completed} / {task.total}</span>
              ...
              <div className="flex gap-3 mt-8">
                <button onClick={() => setIsUpdateProgressOpen(false)} className="flex-1 px-6 py-3 rounded-xl border border-gray-200 text-black font-bold hover:bg-gray-100 transition-all">Cancel</button>
                <button
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setIsRenewModalOpen(false)} className="p-2 hover:bg-black/5 rounded-xl text-gray-800">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-black uppercase tracking-widest block mb-1.5">Package Name</label>
                  <input
                    type="text"
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-black focus:border-purple-500 outline-none transition-all placeholder:text-gray-600"
                ...
                <div>
                  <label className="text-[10px] font-bold text-black uppercase tracking-widest block mb-1.5">Work Detail</label>
                  <textarea
                    rows={4}
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-black focus:border-purple-500 outline-none transition-all placeholder:text-gray-600"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-black uppercase tracking-widest block mb-1.5">Deadline</label>
                    <input
                      type="date"
                      className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-black focus:border-purple-500 outline-none transition-all"
              ...
              <div className="flex gap-3 mt-8">
                <button onClick={() => setIsRenewModalOpen(false)} className="flex-1 px-6 py-3 rounded-xl border border-gray-200 text-black font-bold hover:bg-gray-100 transition-all">Cancel</button>
                <button
        ...
        {isAddTaskModalOpen && selectedProjectForTask && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="relative w-full max-w-lg bg-white rounded-3xl border border-gray-200 p-8 shadow-2xl max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-black flex items-center gap-2">
                  <Plus className="text-blue-500" /> Add New Task
                </h3>
                <button onClick={() => setIsAddTaskModalOpen(false)} className="p-2 hover:bg-black/5 rounded-xl text-gray-800">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-black uppercase tracking-widest block mb-1.5">Task Name</label>
                  <input
                    type="text"
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-black focus:border-blue-500 outline-none transition-all placeholder:text-gray-600"
                ...
                <div>
                  <label className="text-[10px] font-bold text-black uppercase tracking-widest block mb-1.5">Total Quantity</label>
                  <input
                    type="number"
                    min="1"
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-black focus:border-blue-500 outline-none transition-all placeholder:text-gray-600"
                ...
                <div className="flex items-center gap-3 p-4 rounded-2xl bg-gray-50 border border-gray-200">
                  <input
                    ...
                    className="w-5 h-5 rounded bg-blue-100 text-blue-600 border-blue-300 focus:ring-blue-500"
                  />
                  <label htmlFor="isExtraTask" className="text-sm font-bold text-black">Mark as Extra Task</label>
                </div>
              </div>

              <div className="flex gap-3 mt-8">
                <button onClick={() => setIsAddTaskModalOpen(false)} className="flex-1 px-6 py-3 rounded-xl border border-gray-200 text-black font-bold hover:bg-gray-100 transition-all">Cancel</button>
                <button
                  onClick={addNewTask}
                  className="flex-1 px-6 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20"
                >
